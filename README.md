...
- 显示技能版本和关联设备数量
- 提供技能详情查看和删除功能
- 运行计划的「预警名称」会出现在报警列表。新建时会默认填成技能名，建议不要改成别的旧名字

### 模型工厂（ML Pipeline）
- 完整的 AI 模型开发流程：自动采集 → 数据标注 → 数据导出 → 模型训练 → 模型导出 → 模型下载
- 摄像头自动采集：按「画面变化 / 定点抓拍 / 小模型 / 大模型 / 技能编排」模板抓图入库；
  用到画面变化判定时，可在向导第 2 步为每个点位单独绘制检测区域（复用运行计划的 `FenceDrawer`），
  支持多区域与「反选」屏蔽干扰源，未绘制的点位按整幅画面判定；
  第 3 步可选择配额方式：每个点位各一份（默认）或全部点位共用同一份额度；
  采集中的任务会在重启后端后自动继续，只有点「停止」或采满额度才会关掉
- 集成 Label Studio 在线标注工具，支持目标检测标注
- 数据集图片管理按页展示缩略图（每页 48 张），图片很多时也能较快刷出；点开仍看原图
- 支持 YOLO11 / YOLO26 系列模型训练，覆盖 5 种任务类型
- 实时训练进度、训练日志查看，集成 TensorBoard 可视化
- 异步模型导出（ONNX / TensorRT / OpenVINO 等 8 种格式），完成后自动通知
- 模型文件在线下载（训练产出 / 导出模型）
- GPU 设备信息检测与展示
- 组件路径：`src/components/visionAI/mlPipeline/modelFactory.vue`
...
- 支持实时视频流展示
- 多画面同时显示
- 智能识别结果可视化：检测框叠在画面上；围栏外的目标用虚线框，围栏内实线
- 通用运行状态徽标：技能图配了「运动状态」（光流）时，画面左上角显示「节点名 + 运行中/已停止」；没配光流则不显示，不写死某一种设备

### 预警管理
- 实时展示预警信息和处理状态
- 提供预警处理、档案管理、数据导出等功能
- 筛选支持：日期、等级、类型、技能、状态、预警名称/ID、**点位下拉**（选项来自 `/api/v1/alerts/skills` 的 `cameras`，按 `camera_id` 精确筛选；先选技能后，点位只列出该技能出过预警的点位）
- 归档：须先「处理」并结束处理（状态=已处理）；未完成时点归档会弹出提示，不会静默灰掉无反馈；已归档/误报按钮禁用并有悬停说明
...
#### 关键文件
- `src/components/service/VisionAIService.js` - API服务封装（含 mlPipelineAPI）
- `src/components/visionAI/mlPipeline/modelFactory.vue` - 模型工厂页面（标注-训练-导出-下载）
...
- **模型工厂（ML Pipeline）**：完整的标注-训练-导出-下载流程
  - **数据标注**：集成 Label Studio，创建数据集 → 上传图片 → 在线标注 → 同步结果
  - **图片管理**：列表分页 + 缩略图懒加载，避免上千张原图一次性撑爆页面
  - **数据导出**：将标注结果导出为 YOLO 格式 ZIP，支持自定义 train/val 划分比例
  - **模型训练**：选择任务类型（检测/分割/分类/姿态/旋转检测）和基础模型（YOLO11/YOLO26），配置训练参数后启动训练，支持实时进度查看、训练日志、TensorBoard 可视化
  - **训练控制**：支持中断（保留检查点可恢复）、取消（从头重来）、重新训练等操作
  - **模型导出**：训练完成后异步导出为 8 种部署格式（ONNX/TensorRT/OpenVINO 等），导出过程不阻塞 UI，完成后弹窗通知
  - **模型下载**：支持在线下载训练产出（best.pt）和导出后的模型文件
