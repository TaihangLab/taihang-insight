# 太行视觉AI平台 (Taihang Insight)

> 一个基于 Vue 3 + Vite 的现代化可视化监控平台，用于展示算法推理系统的运行状态和数据统计。

## 项目概述

太行视觉AI平台是一个大屏可视化系统，用于实时监控和展示AI算法推理引擎的各项性能指标、设备连接状态、报警事件等信息。该平台采用现代化设计风格，以蓝色科技感为主题，提供直观的数据可视化展示。
...
### UI 框架与样式
- **Element Plus 2.9.2** - Vue 3 组件库
- **UnoCSS 0.66.4** - 原子化 CSS 引擎
  - presetUno - 原子化 CSS 基础预设
  - presetAttributify - 属性化模式支持
  - presetIcons - Iconify 图标支持 (@iconify-json/carbon)
- **CSS Variables** - 统一的设计系统主题变量

### 可视化与 3D
- **ECharts 5.6.0** - 专业数据可视化图表库
- **vue-echarts 7.1.0** - ECharts 的 Vue 3 组件封装
- **data-view-vue3 2.0.0** - 大屏数据可视化组件
- **Three.js 0.137.5** - 3D 图形库

### 视频与媒体
- **@liveqing/liveplayer 2.7.10** - 直播播放器
- **video.js 7.20.3** - HTML5 视频播放器

### 地图与其他
- **OpenLayers 6.14.1** - 开源地图库
- **@femessage/log-viewer 1.5.0** - 日志查看组件
- **@fingerprintjs/fingerprintjs 4.5.1** - 浏览器指纹识别

### HTTP 与工具
- **Axios 1.7.9** - HTTP 客户端
- **Screenfull 6.0.2** - 全屏 API 封装
...
npm run dev
```
开发服务器将在 `http://localhost:4000` 启动，支持热模块替换 (HMR)。
...
### UnoCSS 原子化样式
使用 UnoCSS 进行原子化样式开发：
- `class="flex items-center justify-center"` 代替自定义 CSS
- 使用 Carbon 图标集：`<i class="i-carbon-user"></i>`
- 预设快捷类：`flex-center`、`card`、`btn-primary` 等

### 统一主题系统
基于 CSS Variables 的主题系统：
- Element Plus 主题变量统一
- 设计系统变量（`--primary-color`、`--sidebar-width` 等）
- 支持主题切换和样式覆盖
...
- **Element Plus 2.9.2** - Vue 3 UI 组件库
- **UnoCSS 0.66.4** - 原子化 CSS 引擎
- **Pinia 2.3.0** - 状态管理
- **Axios 1.7.9** - HTTP 客户端
...
- 开发服务器端口：4000
- 别名配置：`@` → `src`，`@static` → `static`
- UnoCSS 插件集成
- 构建分包策略

### API服务配置
在 `src/main.ts` 中配置后端服务地址：
```typescript
const config = {
