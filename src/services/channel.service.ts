...
export class ChannelService {
  ...
  async getChannelList(params?: ChannelListParams): Promise<PaginatedResponse<Channel>> {
    const response = await axiosInstance.get<unknown, PaginatedResponse<Channel>>(this.basePath, {
      params,
    });
    return response;
  }
  ...
  async getChannelDetail(channelId: string): Promise<ApiResponse<Channel>> {
    const response = await axiosInstance.get<unknown, ApiResponse<Channel>>(
  ...
  async playChannel(channelId: string): Promise<ApiResponse<{ stream_url: string }>> {
    const response = await axiosInstance.post<unknown, ApiResponse<{ stream_url: string }>>(
  ...
  async getChannelTree(params?: { catalog_id?: string }): Promise<ApiResponse<any[]>> {
    const response = await axiosInstance.get<unknown, ApiResponse<any[]>>(`${this.basePath}/tree`, {
      params,
    });
    return response;
  }
