...
export class ReviewSkillService {
   ...
   * 预览测试复判技能
   */
  async previewTestReviewSkill(testImage: File, userPrompt: string): Promise<ApiResponse<any>> {
    const formData = new FormData();
  ...
  async updateReviewSkill(
    skillId: string,
    updateData: Partial<CreateReviewSkillParams>,
  ): Promise<ApiResponse<ReviewSkill>> {
    const formData = new FormData();
