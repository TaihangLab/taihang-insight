...
async function loadData(): Promise<void> {
  ...
  try {
    ...
    if (images && images.length > 0) {
      warningImages.value = images.map(
        (item: AlertImage): WarningImage => ({
          id: item.id,
...
function prevImage() {
  ...
  currentIndex.value =
    (currentIndex.value - 1 + warningImages.value.length) % warningImages.value.length;
}
  ...
  <div class="w-full h-full flex flex-col gap-3">
    ...
    <div class="relative flex-1 bg-black/20 rounded-lg overflow-hidden min-h-[200px]">
      <template v-if="currentImage">
        ...
        <!-- 预警信息覆盖层 - 右下角小窗口 -->
        <div
          class="absolute bottom-16 right-3 bg-black/40 backdrop-blur border border-cyan-400/30 rounded-lg px-4 py-3 min-w-[200px]"
        >
      ...
      <div class="absolute top-3 left-3 right-3 flex justify-between pointer-events-none">
        <!-- 今日预警 -->
        <div
          class="flex items-center gap-2 bg-black/40 backdrop-blur border border-cyan-400/30 rounded-lg px-3 py-2"
        >
          <div
            class="w-8 h-8 rounded-full bg-gradient-to-br from-red-500 to-orange-500 flex items-center justify-center"
          >
          ...
          <div>
            ...
            <div class="text-lg font-bold text-cyan-400">
              {{ todayWarnings }}
              <span class="text-xs ml-1">个</span>
            </div>
          </div>
        </div>
        ...
        >
          <div
            class="w-8 h-8 rounded-full bg-gradient-to-br from-cyan-400 to-cyan-500 flex items-center justify-center"
          >
    ...
    <!-- 缩略图列表 -->
    <div
      v-if="warningImages.length > 0"
      class="h-16 flex items-center gap-2 overflow-x-auto overflow-y-hidden"
    >
      <div
