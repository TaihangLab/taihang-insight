      ...
      <template v-else>
        <div class="w-[120px] h-[120px] mb-2.5 relative flex items-center justify-center">
          <svg class="w-full h-full" viewBox="0 0 100 100">
            <circle
              v-for="(section, index) in alarmSectionsWithAngles"
              :key="index"
              cx="50"
              cy="50"
              r="40"
          ...
          <div
            v-if="tooltipVisible"
            class="absolute px-2 py-1 rounded text-xs z-50 whitespace-nowrap"
        ...
        <div class="w-full flex flex-col gap-2">
          ...
          >
            ...
            <span class="text-xs font-bold" style="color: #00ffff">
              {{ alarmSections[index]?.value || 0 }}
            </span>
          </div>
        </div>
      </template>
