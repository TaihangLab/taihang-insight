...
export default {
  ...
  methods: {
    ...
    async fetchAvailableUsers() {
      ...
      try {
        // 构建请求参数
        const skip = (this.userPagination.currentPage - 1) * this.userPagination.pageSize;
        const params = {
          skip: skip,
        ...
        // API调用失败
        console.error('⚠️ API调用失败:', error.message)
        this.$message.error(`获取可选用户列表失败: ${error.message}`)
        // 在API调用失败时清空表格数据
    ...
    async saveAddUser() {
      ...
      try {
        ...
        // 调用API分配角色给用户
        await associationService.assignRolesToUser(userNames, this.role_code, this.searchForm.tenant_id || 'default')
    ...
    async cancelAuth(row) {
      this.$confirm(`确定要取消用户 "${row.user_name}" 的授权吗？`, '确认取消', {
        ...
        try {
          // 调用API取消用户的角色分配
          await associationService.removeUserRole(row.user_name, this.role_code, row.tenant_id || 'default')
          ...
          this.$message.error(`取消授权失败: ${error.message || '未知错误'}`)
        } finally {
          this.loading = false
        }
      }).catch(() => {
      ...
      this.$confirm(`确定要取消选中的 ${this.selectedRows.length} 个用户的授权吗？`, '确认取消', {
        ...
        try {
          ...
          const promises = this.selectedRows.map(row =>
            associationService.removeUserRole(row.user_name, this.role_code, row.tenant_id || 'default')
          );
          ...
          this.$message.error(`批量取消授权失败: ${error.message || '未知错误'}`)
        } finally {
          this.loading = false
        }
      }).catch(() => {
