    ...
    >
      ...
      >
        <el-input
          ...
          :placeholder="
            permissionForm.permission_type === 'button'
              ? '如：/api/v1/rbac/users'
              : '如：/system/user-management'
          "
      ...
      <el-form-item label="权限编码" prop="permission_code">
        ...
        <div class="form-tip">格式：system:user 或 system:user:view</div>
      </el-form-item>
      ...
      >
        <el-input
          ...
          placeholder="如：@/pages/system/userManagement.vue"
        ></el-input>
        <div class="form-tip">支持 @ 别名，相对路径</div>
      </el-form-item>

      <!-- 图标 (仅 folder 和 menu 类型) -->
      <el-form-item v-if="permissionForm.permission_type !== 'button'" label="图标" prop="icon">
      ...
      <el-form-item label="显示选项" v-if="permissionForm.permission_type !== 'button'">
        ...
        <el-checkbox
          v-if="permissionForm.permission_type === 'folder'"
          v-model="permissionForm.layout"
        >
          需要 Layout
        </el-checkbox>
      </el-form-item>
...
const permissionRules: FormRules<PermissionForm> = {
  ...
  path: [
    ...
    {
      pattern: /^[a-zA-Z0-9/{}-]+$/,
      message: "路由路径只能包含字母、数字、斜杠、短横线和路径参数",
      trigger: "blur",
    },
  ],
};
...
const parentOptions = computed((): ParentOption[] => {
  ...
  return [{ id: -1, permission_name: "（作为根节点）" }, ...filterTree(props.permissionTree)];
});
...
const initForm = () => {
  if (isCreate.value) {
    ...
    permissionForm.value = {
      permission_type:
        parentNode.value?.node_type === "folder"
          ? "menu"
          : parentNode.value?.node_type === "menu"
            ? "button"
            : "folder",
      permission_name: "",
      permission_code: "",
...
const submitForm = async () => {
  ...
  try {
    ...
    console.log(
      "[PermissionEditDialog] parent_id:",
      permissionForm.value.parent_id,
      typeof permissionForm.value.parent_id,
    );
    ...
    if (permissionForm.value.parent_id !== null && permissionForm.value.parent_id !== undefined) {
      submitData.parent_id =
        permissionForm.value.parent_id === -1 ? null : permissionForm.value.parent_id;
