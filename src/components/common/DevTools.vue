...
<template>
  <div v-if="enabled">
    ...
    <span>开发工具：</span>
    <el-button size="small" @click="handleFill" type="primary">快速填充测试数据</el-button>
    <el-button size="small" @click="handleClear">清空表单</el-button>
  </div>
</template>
...
const mockDataGenerators = {
  ...
  role: () => ({
    ...
    role_name:
      ["管理员", "普通用户", "访客", "审核员", "编辑"][Math.floor(Math.random() * 5)] +
      Math.floor(Math.random() * 100),
    role_code:
      "ROLE_" + ["ADMIN", "USER", "GUEST", "REVIEWER", "EDITOR"][Math.floor(Math.random() * 5)],
  ...
  department: () => ({
    name:
      ["技术部", "市场部", "财务部", "人事部", "运营部"][Math.floor(Math.random() * 5)] +
      Math.floor(Math.random() * 100),
  ...
  permission: () => {
    ...
    return {
      ...
      permission_name: ["用户管理", "角色管理", "部门管理", "权限管理", "系统设置"][
        Math.floor(Math.random() * 5)
      ],
      permission_code:
        "system:" +
        ["user", "role", "dept", "permission", "setting"][Math.floor(Math.random() * 5)],
      path:
        "/system/" +
        ["user", "role", "dept", "permission", "setting"][Math.floor(Math.random() * 5)],
      component:
        "@/pages/system/" +
        ["userManagement", "roleManagement", "deptManagement", "permissionManagement"][
      ...
      status: [0, 1][Math.floor(Math.random() * 2)],
      method:
        type === "button" ? ["GET", "POST", "PUT", "DELETE"][Math.floor(Math.random() * 4)] : "",
    };
  },
};
