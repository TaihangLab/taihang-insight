  ...
  methods: {
    ...
    saveCurrentChat() {
      ...
      if (this.currentChatId && this.messages.length > 0) {
        const chatIndex = this.chatHistories.findIndex(
          (chat) => chat.conversation_id === this.currentChatId,
        );
        if (chatIndex !== -1) {
          // 更新本地缓存的会话信息
    ...
    updateCurrentChatSelection() {
      if (this.currentChatId && this.chatHistories.length > 0) {
        ...
        this.currentChatIndex = this.chatHistories.findIndex(
          (c) => c.conversation_id === this.currentChatId,
        );
    ...
    onDrag(event) {
      if (this.isDragging) {
        ...
        // 限制在窗口范围内
        this.tempPosition.x = Math.max(
          10,
          Math.min(this.tempPosition.x, windowWidth - ballSize - 10),
        );
        this.tempPosition.y = Math.max(
          10,
          Math.min(this.tempPosition.y, windowHeight - ballSize - 10),
        );
      }
    },
    stopDrag() {
        ...
        for await (const chunk of centerAPI.chatAssistant.createChatStream(chatData, {
          ...
          if (chunk.done) {
            ...
            if (this.messages.length === 2 && this.currentChatId) {
              try {
                const response = await centerAPI.chatAssistant.autoGenerateTitle(
                  this.currentChatId,
                );
    ...
    parseThinkingState(message, fullResponse) {
      ...
      if (thinkStartIndex !== -1) {
        ...
        if (thinkEndIndex !== -1) {
          ...
          message.thinkingContent = fullResponse
  ...
  <div class="think-content" style="${contentStyle}">${thinkContent.replace(/\n/g, "<br>")}</div>
    ...
    bindThinkBlockEvents() {
      ...
      this.$nextTick(() => {
        ...
        if (this.$refs.fullscreenMessagesContainer) {
          this.$refs.fullscreenMessagesContainer.removeEventListener(
            "click",
            this.handleThinkToggle,
          );
    ...
    scrollToBottom() {
      this.$nextTick(() => {
        ...
        if (this.$refs.fullscreenMessagesContainer) {
          this.$refs.fullscreenMessagesContainer.scrollTop =
            this.$refs.fullscreenMessagesContainer.scrollHeight;
        }
