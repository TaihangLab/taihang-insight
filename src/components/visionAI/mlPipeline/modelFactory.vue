  ...
  <div class="model-factory">
      ...
      <div class="right-panel">
        ...
        <div v-else class="detail-content">
          ...
          <el-alert
            ...
            :title="lsProjectWarning || '此数据集未关联标注项目'"
            type="warning"
                ...
                <div class="anno-layout">
                  ...
                  <div class="anno-card anno-card--actions">
                    <div class="anno-action">
                      <h4>去标注</h4>
                      <p class="anno-hint">用当前平台账号打开。审核时点进已保存的图，编辑器上有「通过 / 驳回」。</p>
                      <el-button
                        v-if="selectedDataset.ls_project_id"
                        type="primary"
                        size="medium"
    ...
    <el-dialog title="新建数据集" :visible.sync="createDialogVisible" width="440px" :close-on-click-modal="false" append-to-body>
      ...
      <div class="form-tip" style="padding: 0 20px; color: #909399; font-size: 12px;">
        创建后请在标注项目中设置标注类型和标注类别，然后上传图片开始标注。
      </div>
      <div slot="footer">
        ...
        if (data.already_exists) {
          ...
          if (failed > 0) {
            this.$message.warning(
              `标注项目已重建。成功重新导入 ${reimported} 张，${failed} 张原图已随旧项目丢失，请重新上传。`
            );
          } else {
