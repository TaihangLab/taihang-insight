            ...
            <div class="page-title-container">
              ...
              <span v-if="showSkillInfo" class="skill-info">
                ...
                <span class="skill-status">
                  <span
                    class="status-dot"
                    :class="skillData.status === 'online' ? 'online' : 'offline'"
                  ></span>
                  {{ skillStatus }}
                </span>
              </span>
            ...
            <div class="params-card">
              ...
              <div class="form-section">
                ...
                <div class="form-item">
                  ...
                  <div class="form-tips" v-if="!isViewMode">可添加五个标签，支持多选</div>
                  <div class="form-tips" v-else>
                    当前技能标签：{{
                      skillForm.categories && skillForm.categories.length > 0
                        ? skillForm.categories.join("、")
                        : "无"
                    }}
                  </div>
                </div>
      ...
      <div class="example-panel" @click.stop>
        <div class="example-header">
          ...
          <el-button
            icon="el-icon-close"
            link
            @click="closeExamplePanel"
            class="close-btn"
          ></el-button>
        </div>

        <div class="example-subtitle">您可以通过以下方式描述技能，或直接引用示例查看效果：</div>
...
export default {
  ...
  computed: {
    canAnalyze() {
      return this.skillForm.name && this.skillForm.description && this.fileList.length > 0;
    },
  ...
  methods: {
    ...
    async loadSkillData(skillId) {
      try {
        ...
        if (skill) {
          ...
          if (this.isViewMode) {
            this.analysisResult =
              "该技能已配置完成，能够有效识别图像中的目标对象，置信度达到95%以上。";
          }
    ...
    useExample(example) {
      ...
      this.$confirm(
        `确认引用"${example.title}"示例吗？这将替换当前的技能名称、描述和标签`,
        "引用示例",
        {
