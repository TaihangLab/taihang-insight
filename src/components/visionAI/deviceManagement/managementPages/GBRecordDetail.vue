          ...
          <div>
            ...
            <el-button-group>
              ...
              <el-dropdown size="small" title="播放倍速" @command="gbScale">
                ...
                <el-dropdown-menu>
                  <el-dropdown-item
                    v-for="(item, index) in downloadSpeedArray"
                    :key="index"
                    :command="item"
                  >
                    {{ item }}倍速
...
export default {
  ...
  methods: {
    ...
    setTime: function (startTime, endTime) {
      ...
      let start =
        (new Date(this.startTime).getTime() - new Date(this.chooseDate + " 00:00:00").getTime()) /
        1000;
      let end =
        (new Date(this.endTime).getTime() - new Date(this.chooseDate + " 00:00:00").getTime()) /
        1000;
      console.log(start);
      console.log(end);
    ...
    downloadRecord: function (row) {
      if (!row) {
        let startTimeStr = moment(
          new Date(this.chooseDate + " 00:00:00").getTime() + this.playTime[0] * 1000,
        ).format("YYYY-MM-DD HH:mm:ss");
        let endTimeStr = moment(
          new Date(this.chooseDate + " 00:00:00").getTime() + this.playTime[1] * 1000,
        ).format("YYYY-MM-DD HH:mm:ss");
      ...
      if (this.streamId !== "") {
        ...
        wvpAxios({
          ...
          url:
            ...
            row.endTime +
            "&downloadSpeed=" +
            this.downloadSpeedArray[this.downloadSpeedArray.length - 1],
        }).then((res) => {
          if (res.data.code === 0) {
    ...
    stopDownloadRecord: function (callback) {
      ...
      wvpAxios({
        method: "get",
        url:
          "/gb_record/download/stop/" + this.deviceId + "/" + this.channelId + "/" + this.streamId,
      }).then((res) => {
    ...
    getDataLeft(item) {
      let timeForFile = this.getTimeForFile(item);
      let differenceTime =
        timeForFile[0].getTime() - new Date(this.chooseDate + " 00:00:00").getTime();
      return (
        parseFloat(
          (differenceTime - this.sliderMIn * 1000) / ((this.sliderMax - this.sliderMIn) * 1000),
        ) * 100
      );
    },
    getUrlByStreamInfo() {
      ...
      if (this.sliderMIn === 0 && this.sliderMax === 86400) {
        if (this.detailFiles.length > 0) {
          ...
          let timeNum =
            timeForFile[0].getTime() - new Date(this.chooseDate + " " + "00:00:00").getTime();
          let lastTimeNum =
            lastTimeForFile[1].getTime() - new Date(this.chooseDate + " " + "00:00:00").getTime();

          this.playTime = parseInt(timeNum / 1000);
          this.sliderMIn = parseInt(timeNum / 1000 - ((timeNum / 1000) % (60 * 60)));
          this.sliderMax =
            parseInt(lastTimeNum / 1000 - ((lastTimeNum / 1000) % (60 * 60))) + 60 * 60;
