      ...
      <template #content>
        <div class="header-content">
          ...
          <div class="header-text">
            <h2>本地视频管理</h2>
            <span class="subtitle">上传本地视频并推流到RTSP服务器，实现虚拟摄像头功能</span>
          </div>
        </div>
      </template>
      <template #extra>
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column label="推流状态" width="120" align="center">
          <template #default="{ row }">
            ...
            <el-tag v-else type="info" effect="plain">
              <el-icon class="tag-icon"><VideoPause /></el-icon>
              未推流
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="RTSP地址" min-width="250">
          <template #default="{ row }">
            <div v-if="row.is_streaming && (row as any).rtsp_url" class="rtsp-url-cell">
        ...
        <el-table-column label="操作" width="280" align="center" fixed="right">
          <template #default="{ row }">
            <el-space wrap>
              ...
              <el-button size="small" :icon="Edit" @click="showEditDialog(row)">
                编辑
              </el-button>
              <el-button
                type="danger"
    ...
    >
      ...
      >
        <el-form-item label="推流ID">
          ...
          />
          <div class="form-tip">推流ID用于构建RTSP地址（如：rtsp://.../推流ID），留空则自动生成</div>
        </el-form-item>
        <el-form-item label="推流帧率">
          <el-space>
            <el-input-number
...
.stat-card-success :deep(.el-statistic__head) {
