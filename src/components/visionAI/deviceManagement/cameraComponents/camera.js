...
export default {
  ...
  data() {
    return {
      ...
      // 表单验证规则
      rules: {
        name: [{ max: 50, message: "任务名称不能超过50个字符", trigger: "blur" }],
        description: [{ max: 200, message: "任务描述不能超过200个字符", trigger: "blur" }],
        alarmLevel: [
          {
  ...
  computed: {
    ...
    currentAlarmDescription() {
      ...
      const selectedOption = this.alarmLevelOptions.find(
        (option) => option.value === this.skillForm.alarmLevel,
      );
  ...
  methods: {
    ...
    removeSelectedSkill(skill) {
      this.skillSelectForm.selectedSkills = this.skillSelectForm.selectedSkills.filter(
        (s) => s !== skill,
      );
    },
    ...
    configureSkill(skill, isNewSelection = true) {
      ...
      setTimeout(() => {
        ...
        this.skillForm = {
          ...
          electronicFence: {
            image:
              "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=", // 默认空白背景图
            points: [],
            isDrawing: false,
            triggerMode: "inside",
            tempPoints: [],
        ...
        // 获取当前技能的完整信息
        const skillInfo = this.skillOptions.find(
          (option) => option.value === skill || option.name_zh === skill,
        );
        ...
        // 从API获取该技能类的默认参数
        centerAPI.skill
          .getAITaskSkillDetail(skillInfo.id)
          .then((response) => {
            // 响应拦截器已处理成功/失败判断，直接使用数据
            let params = {};
            ...
            if (
              skillDetail &&
              skillDetail.default_config &&
              skillDetail.default_config.alert_definitions
            ) {
    ...
    getAndSetCameraSnapForFence(cameraId) {
      ...
      getSnapPromise
        .then((result) => {
          ...
          console.log(
            "获取截图成功，图像大小:",
            result.imageData.size,
            "类型:",
            result.imageData.type,
          );
            ...
            if (deviceIndex !== -1 || this.isUpdateMode) {
              ...
              const taskData = {
                ...
                name:
                  this.skillForm.name ||
                  (deviceIndex !== -1
                    ? `${this.deviceList[deviceIndex].name}-${skillKey}分析任务`
                    : `${skillKey}分析任务`),
                ...
                description:
                  ...
                  (deviceIndex !== -1
                    ? `此任务使用${skillKey}技术对${this.deviceList[deviceIndex].name}进行智能分析，可自动识别并进行预警`
                    : `此任务使用${skillKey}技术进行智能分析，可自动识别并进行预警`),
                ...
                skill_config: {
                  params:
                    this.skillDetailData && this.skillDetailData.params
                      ? this.skillDetailData.params
                      : {},
                },
                // 实时推流配置
                config: {
                ...
                centerAPI.skill
                  ...
                  .then((response) => {
                    ...
                    if (
                      skillDetail &&
                      skillDetail.default_config &&
                      skillDetail.default_config.params
                    ) {
    ...
    saveSkillDetails() {
      ...
      if (this.currentSkill && this.currentDeviceId) {
        ...
        if (deviceIndex !== -1) {
          ...
          // 查找匹配的技能信息
          const skillInfo = this.skillOptions.find(
            (option) => option.value === this.currentSkill || option.name_zh === this.currentSkill,
          );
    ...
    getSkillIcon(skill) {
      // 查找当前技能在技能选项中的配置
      const skillOption = this.skillOptions.find(
        (option) => option.value === skill || option.name_zh === skill,
      );
    ...
    fetchSkillClasses(params = {}) {
      ...
      centerAPI.skill
        .getAITaskSkillClasses(queryParams)
        .then((response) => {
    ...
    formatFencePoints(points) {
      ...
      if (Array.isArray(points[0])) {
        return points.map((polygon) => polygon.map((point) => ({ x: point.x, y: point.y })));
      }
      ...
      this.$alert(
        "图像加载失败，可能原因：\n1. 摄像头不在线\n2. 摄像头未返回有效图像\n3. 网络问题\n\n请尝试刷新或稍后重试",
        {
          ...
          callback: () => {
            ...
            // 设置默认占位图
            this.skillForm.electronicFence.image =
              "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=";
          },
        },
      );
    ...
    completeFence() {
      ...
      // 将当前绘制的多边形添加到点数组中
      this.skillForm.electronicFence.points.push([
        ...this.skillForm.electronicFence.currentPolygon,
      ]);
      ...
      centerAPI.skill
        ...
        .then((response) => {
          ...
          if (responseData && responseData.id) {
            ...
            if (
              this.tempConfiguredSkill &&
              !this.skillSelectForm.selectedSkills.includes(this.tempConfiguredSkill)
            ) {
        ...
        centerAPI.skill
          ...
          .then((response) => {
            ...
            if (taskData.skill_class_id) {
              // 查找对应的技能信息
              centerAPI.skill
                .getAITaskSkillDetail(taskData.skill_class_id)
                .then((skillData) => {
                  // 响应拦截器已处理成功/失败判断，直接使用数据
                  ...
                  if (skillData.default_config && skillData.default_config.alert_definitions) {
                    ...
                    console.log(
                      "从编辑任务中加载预警定义:",
                      skillData.default_config.alert_definitions,
                    );
                  } else {
    ...
    fillSkillFormFromTask(taskData) {
      ...
      // 填充运行时段
      if (
        taskData.running_period &&
        taskData.running_period.periods &&
        taskData.running_period.periods.length > 0
      ) {
      ...
      if (
        taskData.electronic_fence &&
        taskData.electronic_fence.points &&
        taskData.electronic_fence.points.length > 0
      ) {
        // 判断电子围栏点数据的格式
        ...
        try {
          ...
          console.log(
            "处理后的电子围栏数据:",
            JSON.stringify(this.skillForm.electronicFence.points),
          );
        } catch (error) {
        ...
        // 设置触发模式
        this.skillForm.electronicFence.triggerMode =
          taskData.electronic_fence.trigger_mode || "inside";
      }
      ...
      this.skillDetailData = {
        ...
        params:
          taskData.skill_config && taskData.skill_config.params
            ? { ...taskData.skill_config.params }
            : {},
      };
    ...
    updateAITask(taskData, deviceIndex, skillKey, config) {
      ...
      // 调用API更新AI任务
      centerAPI.skill
        .updateAITask(this.currentTaskId, taskData)
        .then((response) => {
