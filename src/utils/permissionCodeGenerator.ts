...
class PermissionCodeGenerator {
   ...
   * 根据路由路径生成权限码
   * 规则：模块:页面:操作
   * @param routePath - 路由路径，如 /systemManage/userManagement
   * @param nodeType - 节点类型：folder/menu/button
   * @param action - 操作类型：view/read/create/update/delete等
   * @param parentCode - 父节点权限码（用于生成子节点权限码）
   * @returns 生成的权限码
   */
   ...
   * 从权限码推断节点类型
   * @param code - 权限码
   * @returns 节点类型：folder/menu/button
   */
